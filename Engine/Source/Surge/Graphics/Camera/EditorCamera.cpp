...
namespace Surge
{
    EditorCamera::EditorCamera(float fov, float aspectRatio, float nearClip, float farClip) : Camera(glm::perspective(glm::radians(fov), aspectRatio, nearClip, farClip))
    {
        ...
        mWorldRotation = glm::eulerAngles(orientation) * (180.0f / (float)PI);
    ...
    {
        const float x = std::min(float(mViewportWidth) / 1000.0f, 2.2f);
        const float xFactor = 0.08f * (x * x) - 0.1778f * x + 0.3021f;
    ...
    float EditorCamera::ZoomSpeed() const
    {
        ...
        speed = std::min(speed, 300.0f); // max speed = 300
        return speed;
    }
