                    ...
                    if(selectedPassIndex >= 0)
                    {
                        ...
                        for(auto& group : mCompiledGraph.Groups)
                        {
                            for(auto* pass : group.Passes)
                            {
                                ...
                                {
                                    ImGui::TextColored(ImVec4(1.0f, 0.6f, 0.1f, 1.0f), "PASS: %s", pass->GetName().c_str());
                                    ImGui::Separator();
                                    constexpr float imageSize = 200.0f;
                                    if(ImGui::BeginTable("PassResources", 2, ImGuiTableFlags_BordersInnerV | ImGuiTableFlags_Resizable))
                                    {
